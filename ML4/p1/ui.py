...
root.title("Insurance Checker according to age")

def calculate_consumption():
    try:
        slope = float(slope_entry.get())
        consumption = model.predict(poly.fit_transform([[slope]]))
        result_label.config(text=f" { consumption[0]:.2f} L")
    except ValueError as e:
        result_label.config(text=f"Error: {e}")
    except Exception as e:
        result_label.config(text="An unexpected error occurred.")

Label(root, text="Insurance Checker according to age", font=("Arial", 16)).pack(pady=10)
